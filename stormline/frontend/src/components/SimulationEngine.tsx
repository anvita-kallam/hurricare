          ...
          <div className="max-w-[1920px] mx-auto p-6 space-y-6">
                  ...
                  <div className="space-y-2 text-xs font-exo">
                    <div>
                      ...
                      <div className="text-cyan-200 font-orbitron">
                        {userPlan.allocations.reduce((sum, a) => sum + (a.coverage_estimate?.people_covered || 0), 0).toLocaleString()}
                      </div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <div className="text-cyan-200 font-orbitron">
                        {(userPlan.allocations.reduce((sum, a) => sum + (a.coverage_estimate?.coverage_ratio || 0), 0) / userPlan.allocations.length * 100).toFixed(1)}%
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <table className="w-full text-xs font-exo">
                  ...
                  <tbody>
                    {realPlan.allocations.map(realAlloc => {
                      const userAlloc = userPlan.allocations.find(a => a.region === realAlloc.region)
                      const mlAlloc = mlPlan.allocations.find(a => a.region === realAlloc.region)
            ...
            <button
              onClick={() => {
                ...
                a.href = url
                a.download = `stormline_comparison_${selectedHurricane?.id}_${Date.now()}.json`
                a.click()
